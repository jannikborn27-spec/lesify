...
| Wartungs-Jobs          | `pnpm --filter @lesify/api job <name>` (Aufbewahrung, Usage-Historie, Token-Hygiene, Abo-Änderungen), JSON-Summary, Exit-Code                   | Scheduler + Job-Monitoring in Phase 16                                                                 |
| KI-Kosten              | —                                                                                                                                               | Phase 6: `response.usage` je Call-Typ loggen, gegen `PLAN_ECONOMICS` aggregieren, Alarm bei Ausreißern |
